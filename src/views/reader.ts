...
function bindReaderEvents(root: HTMLElement, id: number, initialSaved: string[]) {
  ...
  bindParagraphActions(article, blocks, chunks, initialSaved, id);
  ...
  article.addEventListener('click', (e) => {
    if ((e.target as HTMLElement).closest('.para-actions')) return;
...
const SAVE_ICON_FILLED = `<svg class="icon" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>`;
const SAVE_ICON_OUTLINE = `<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>`;
...
function renderParaActionButtons(isSaved: boolean): string {
  return (
    `<button type="button" class="para-actions__btn${isSaved ? ' para-actions__btn--saved' : ''}" data-para-action="save" aria-label="${isSaved ? '保存を解除' : '保存'}" aria-pressed="${isSaved}">${isSaved ? SAVE_ICON_FILLED : SAVE_ICON_OUTLINE}</button>` +
    `<button type="button" class="para-actions__btn" data-para-action="copy" aria-label="コピー">${COPY_ICON}</button>`
  );
}

function bindParagraphActions(
  article: HTMLElement,
  blocks: HTMLElement[],
  chunks: string[],
  initialSaved: string[],
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    span.innerHTML = renderParaActionButtons(saved.has(text));
    block.appendChild(span);
  }

  article.addEventListener('click', async (e) => {
    const btn = (e.target as HTMLElement).closest<HTMLButtonElement>('.para-actions__btn');
    if (!btn) return;
    e.stopPropagation();
    e.preventDefault();
    ...
    if (action === 'copy') {
      ...
      if (wrap) wrap.innerHTML = renderParaActionButtons(saved.has(text));
